...
- To assign Azure RBAC roles, you must have the `Microsoft.Authorization/roleAssignments/write` permission to an Azure subscription in order to assign roles on that subscription. This permission is part of the [Owner](/azure/role-based-access-control/built-in-roles.md) or [User Access Administrator](/azure/role-based-access-control/built-in-roles.md) built in roles.
