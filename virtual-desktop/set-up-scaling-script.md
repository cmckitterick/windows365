...
- A user with the [*Contributor*](/azure/role-based-access-control/role-assignments-portal) role-based access control (RBAC) role assigned on the Azure subscription to create the resources. You'll also need the *Application administrator* and/or *Owner* RBAC role to create a managed identity.
