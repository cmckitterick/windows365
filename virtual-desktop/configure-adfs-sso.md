...
- You must deploy the **Active Directory Federation Services (AD FS)** role. All servers running this role must be domain-joined, have the latest Windows updates installed, and be running Windows Server 2016 or later. See our [federation tutorial](/entra/identity/hybrid/tutorial-federation.md) to get started setting up this role.
...
- You must deploy **Microsoft Entra Connect** to sync users to Microsoft Entra ID. Microsoft Entra Connect must be configured in [federation mode](/entra/identity/hybrid/how-to-connect-install-custom.md).
