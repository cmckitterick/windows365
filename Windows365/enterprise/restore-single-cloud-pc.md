...
2. Select **Restore** > under **Select restore point**, select the point that you want to restore the Cloud PC to > **Select**. Unhealthy snapshots are indicated in with the ![Image of unhealthy restore point warning icon](./media/restore-overview/triangle.png) icon. Unhealthy snapshots can be used for restoring, but have a lower probability of a successful restore.
